...
impl MovePackage {
    ...
    pub(crate) async fn query(
        ...
        // MovePackages are immutable objects, and thus we can always pass None for checkpoint_sequence_number
        let Some(object) = Object::query(db, address, version, None).await? else {
            return Ok(None);
        };
